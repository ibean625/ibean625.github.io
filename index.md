---
layout: homepage
---

# About Me

Hi, I am a master’s student in Artificial Intelligence at [Tsinghua University](https://www.sigs.tsinghua.edu.cn/sjyxxyjy/list.htm).  I am currently conducting research at the [Tsinghua AI & Robot Lab](https://www.sigs.tsinghua.edu.cn/lhd/list.htm), advised by Prof. Houde Liu（刘厚德）. I received my bachelor’s degree in Robotics Engineering from [Harbin Institute of Technology, Shenzhen](http://smea.hitsz.edu.cn/), where I was advised by Prof. Wenfu Xu（徐文福）and Prof. Jiaole Wang（王焦乐）.

In parallel, I am a co-founder of the robotics startup [ZERITH](https://zerith.ai/), where I have led the Humanoid Robot, Motion Control, and VLA Post-Training departments. In these roles, I direct the development of motion-control algorithms and architect the reinforcement learning-based locomotion and teleoperation frameworks, overseeing end-to-end system integration from algorithm design to real-world deployment.

My research interests include reinforcement learning (RL), vision-language-action (VLA) post-training, and imitation learning (IL), with a focus on embodied intelligence and humanoid control.

<span style="color:#e74d3c !important;"><strong>I am currently seeking Ph.D. opportunities starting in Fall 2027.</strong></span>

## Education and Work

- **ZERITH (零次方)**, *Co-Founder* <br>
  **04/2024 - Present** | Leading Motion Control, Humanoid Robot & VLA Post-Training Departments

- **Tsinghua University (THUsz)**, *Master in Artificial Intelligence* <br>
  **09/2024 - Present** | Advised by Prof. Houde Liu

- **Harbin Institute of Technology, Shenzhen (HITsz)**, *B.Eng. in Robotics Engineering* <br>
  **09/2020 - 06/2024** | **Rank: 1/67**  | Advised by Prof. Wenfu Xu & Prof. Jiaole Wang

{% include_relative _includes/publications.md %}

## Selected Honors and Awards

- **Venture50**, *for ZERITH*, **2025**
- **Forbes China Excellence in Family Services Award**, *for ZERITH*, **2024**
- **GGII Golden Globe Award**, *for ZERITH*, **2024**
- **BDA Product Design Award**, *for ZERITH*, **2024**
- **Global 4th Place**, *IEEE ICRA Quadruped Robot Challenge (QRC)*, **2024**
- **Outstanding Graduate**, *HITSZ*, **2024**
- **Dean's Scholarship** (Top 1%), *HITSZ*, **2024**
- **Huawei Scholarship**, *HUAWEI*, **2023**
- **First-Class Academic Scholarship** (Top 5%), *HITSZ*, **2022**
- **National First Prize**, *15th CAD Competition*, **2022**
- **National Second Prize**, *24th CRAIC Competition*, **2022**
- **First Prize**, *Mathematics Competition*, **2022**
- **National Scholarship**, *Ministry of Education*, **2021**
